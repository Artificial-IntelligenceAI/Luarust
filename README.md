...
Luarust's method is focused on compile once, run anywhere (Like Java's), and the project picks how that file runs: the **bytecode VM**, or the **whole-chunk JIT**, with a **hot JIT** and **native** output planned. **LLVM** does the code generation. The final product only contains what is needed (unlike fucking `Go`, joking, again 😂)
...
| | 10M | 100M | ratio |
| --- | --- | --- | --- |
| C, clang -O2 | 41 ms | 266 ms | 6.5× |
| Rust, rustc -O | 41 ms | 266 ms | 6.5× |
| Luarust JIT | 50 ms | 278 ms | 5.6× |
| Java 21 | 57 ms | 283 ms | 5.0× |
| PyPy 7.3.23 | 56 ms | 313 ms | 5.6× |
| Lua 5.5.1 | 56 ms | 414 ms | 7.4× |
| LuaJIT 2.1 | 61 ms | 443 ms | 7.3× |
| Luarust VM | 132 ms | 1,125 ms | 8.5× |
| Luarust tree-walker | 284 ms | 2,605 ms | 9.2× |
| CPython 3.13 | 414 ms | 3,839 ms | 9.3× |
